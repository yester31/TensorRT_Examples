...
def dataset_load2(batch_size, transform, data_type):
    shuffle = False
    if data_type == 'train':
        shuffle = True
...
def gen_test_data(count=10, show=False):
    ...
    for idx in range(count):
        ...
        print(f"Label: {label}")
        print(f"text: {text}")
        print(f"text: {label2text[str(label)]}")
        
        # PIL → numpy, RGB → BGR
        img_cv2 = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        print(f"image shape: {img_cv2.shape}")
        cv2.imwrite(f"{image_dir_path}/test_{idx}.png", img_cv2)

        if show:
            cv2.imshow(f"test_{idx}", img_cv2)
            cv2.waitKey(0)
