...
def main():
    ...
    qat_onnx = True # whether this onnx from qat_onnx_export.py
    model_name = "resnet18"
    if qat_onnx:
        precision = "int8" 
        onnx_model_path = os.path.join(current_directory, 'onnx', f'{model_name}_{device.type}_qat_bf.onnx') #bf before qat
        engine_file_path = os.path.join(current_directory, 'engine', f'{model_name}_{precision}_qat_bf.engine')
        # onnx_model_path = os.path.join(current_directory, 'onnx', f'{model_name}_{device.type}_qat.onnx') # w batch folding
        # engine_file_path = os.path.join(current_directory, 'engine', f'{model_name}_{precision}_qat.engine')
        # Average FPS: 5623.17 [fps]<- w batch folding
        # Average FPS: 4846.59 [fps]<- wo batch folding
    else :
