...
def main():
    ...
    print(f'[TRT] max output shape : {max_output_shapes}')

    # Load or build the TensorRT engine and do inference
    engine = get_engine(onnx_model_path, engine_file_path, precision, dynamic_input_shapes)
    context = engine.create_execution_context()
    inputs, outputs, bindings, stream = common.allocate_buffers(engine, max_output_shapes, profile_idx=0)
            
            
    # Input
    image_file_names = ['0014.jpg', '0030.jpg']
    for i_idx, image_file_name in enumerate(image_file_names):
        print(f'[TRT] => {image_file_name}')
        ...
        print(f'[TRT] image shape : {batch_images.shape}')
        ...
        # new output shape    
        output_shapes = (1, 3, batch_images.shape[2]*4, batch_images.shape[3]*4)
