...
def main():
    ...
    # Input
    img_path = f'{CUR_DIR}/data/000000039769.jpg'
    ...
    class_queries_logits_shapes = (1, 200, 134)
    masks_queries_logits_shapes = (1, 200, 160, 160)
    ...
    class_queries_logits = trt_outputs[0].reshape(class_queries_logits_shapes)
    masks_queries_logits = trt_outputs[1].reshape(masks_queries_logits_shapes)
    class_logits_per_layer = torch.from_numpy(class_queries_logits).to(DEVICE)
    mask_logits_per_layer =  torch.from_numpy(masks_queries_logits).to(DEVICE)

    mask_logits = interpolate(mask_logits_per_layer, (640, 640), mode="bilinear")
    mask_logits = revert_resize_and_pad_logits_instance_panoptic(mask_logits, [(h, w)])
    ...
    plot_panoptic_results(image, sem_pred, inst_pred, f"{CUR_DIR}/results/{model_name}_000000039769_trt.jpg")
