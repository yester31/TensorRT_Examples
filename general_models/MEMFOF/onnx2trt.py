# by yhpark 2025-8-9
# memfof TensorRT model generation
import os
import sys
...
def main():
    ...
    # input & output shapes 
    output_shape = (1, 2, 2, input_h, input_w)
    print(f'[MDET] "flow" shape : {output_shape}')

    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(f"{CUR_DIR}/results/video_memfof_trt.mp4", fourcc, 20, (640, 480))
    ...
    with get_engine(onnx_model_path, engine_file_path, precision) as engine, \
        ...
        # Inference loop
        for imfile1, imfile2, imfile3 in zip(image_paths[:-2], image_paths[1:-1], image_paths[2:]):
            image1 = load_image(imfile1, (input_w, input_h)) # [1, C, H, W]
            ...
            flow_up = trt_outputs[0].reshape(output_shape)  # [1, 2, 2, H, W]
            flow_up = flow_up[:, 1]                         # [1, 2, H, W]
            flow_up = np.transpose(flow_up[0], (1, 2, 0))   # [H, W, 2]
            flow_img_bgr = flow_to_image(flow_up, convert_to_bgr=True, rad_min=0.02 * (input_h ** 2 + input_w ** 2) ** 0.5)

            image = np.transpose(image2[0], (1, 2, 0)) 
            image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

            img_flo = np.concatenate([image_bgr, flow_img_bgr], axis=0)
