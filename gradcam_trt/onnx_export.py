...
def main():
    ...
    # extract fc weight and save
    weights_fc_w = model.state_dict()['fc.weight'].cpu().numpy()
    weights_fc_w_path = os.path.join(current_directory, 'onnx', f'{model_name}_fc_weights.bin') 
    weights_fc_w.tofile(weights_fc_w_path)
    loaded_weights_fc_w = np.fromfile(weights_fc_w_path, dtype=np.float32).reshape(weights_fc_w.shape)
    print('shape of the fc weights : {}'.format(weights_fc_w.shape)) 
    print('extract the fc weights success : {}'.format(np.array_equal(weights_fc_w, loaded_weights_fc_w))) 
