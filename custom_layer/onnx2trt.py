...
def get_engine(onnx_file_path, engine_file_path="", precision='fp32', gen_engine =False):
...
def main():
    ...
    input_image = cv2.imread(img_path)  # Load image
    #input_image = preprocess_image(img)  # Preprocess image
    
    # Model and engine paths
    precision = "fp32"  # Choose 'fp32' or 'fp16'
    model_name = "resnet18"
    ...
    with get_engine(onnx_model_path, engine_file_path, precision, gen_engine = True) as engine, \
            engine.create_execution_context() as context:

        inputs, outputs, bindings, stream = common.allocate_buffers(engine)
        #inputs[0].host = input_image
        inputs[0].host = input_image.astype(trt.nptype(trt.float32))
